...
class Oracle extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            $conn = $pconnect ? 'oci_pconnect':'oci_connect';//xiao.yingjie 2017-03-12 modify
//            $this->linkID[$linkNum] = $conn($config['username'], $config['password'],$config['database']);//modify by wyfeng at 2008.12.19
            $this->linkID[$linkNum] = $conn($config['username'], $config['password'],'//'.$config['hostname'].'/'.$config['database']);//xiao.yingjie 2017.03.12
