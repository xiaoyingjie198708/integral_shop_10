...
//        $last_sql = htmlspecialchars($this->getLastSql(),ENT_QUOTES);
//        $sql = "INSERT INTO `tm_admin_operation_log` (`user_id`,`user_name`,`operation`,`operation_title`,`operation_sql`,`create_time`) VALUES (".session('admin_uid').",'".session('admin_username')."','添加','".CONTROLLER_NAME."/".ACTION_NAME."','".$last_sql."','".time()."')";
//        $this->db->execute($sql);
    }
    ...
    public function addAll($dataList,$options=array(),$replace=false){
        ...
        $last_sql_zc = $result['sql'];
        $result = $result['result'];
        if(false !== $result ) {
            $this->_after_insertAll($last_sql_zc); //批量添加回调函数 2014/12/23 zhangchong
            $insertId   =   $this->getLastInsID();
            if($insertId) {
                return $insertId;
            }
    ...
    // 批量插入成功后的回调方法 2014/12/23 zhangchong
    protected function _after_insertAll($last_sql) {
        $last_sql = htmlspecialchars($last_sql,ENT_QUOTES);
        $sql = "INSERT INTO `tm_admin_operation_log` (`user_id`,`user_name`,`operation`,`operation_title`,`operation_sql`,`create_time`) VALUES (".session('admin_uid').",'".session('admin_username')."','批量添加','".CONTROLLER_NAME."/".ACTION_NAME."','".$last_sql."','".time()."')";
        $this->db->execute($sql);
    }
    ...
    protected function _after_update($data,$options) {
        $last_sql = htmlspecialchars($this->getLastSql(),ENT_QUOTES);
        $sql = "INSERT INTO `tm_admin_operation_log` (`user_id`,`user_name`,`operation`,`operation_title`,`operation_sql`,`create_time`) VALUES (".session('admin_uid').",'".session('admin_username')."','修改','".CONTROLLER_NAME."/".ACTION_NAME."','".$last_sql."','".time()."')";
        $this->db->execute($sql);
    }
    ...
    protected function _after_delete($data,$options) {
        $last_sql = htmlspecialchars($this->getLastSql(),ENT_QUOTES);
        $sql = "INSERT INTO `tm_admin_operation_log` (`user_id`,`user_name`,`operation`,`operation_title`,`operation_sql`,`create_time`) VALUES (".session('admin_uid').",'".session('admin_username')."','删除','".CONTROLLER_NAME."/".ACTION_NAME."','".$last_sql."','".time()."')";
        $this->db->execute($sql);
    }
