...
class LabelDetailReportController extends BaseController{
    
    public function index(){
        $where = $this->get_where();
        $count = count(M('goods_use_detail_info')->where($where)->group('order_goods_id')->field('id')->select());
        $page = new \Org\My\Page($count, 10);
        $list = M('goods_use_detail_info')->where($where)->group('order_goods_id')->limit($page->firstRow . ',' . $page->listRows)->order('pay_time desc')->select();
    ...
    public function export(){
        ...
        $list = $export_model->where($where)->group('order_goods_id')->order('pay_time desc')->getExportData();
        $shops_codes = getFieldsByKey($list, 'shops_code');
        $shops_names = M('shops_base_info')->where(array('shops_code'=>array('in',$shops_codes)))->getField('shops_code,shops_name',true);
        foreach($list as $k=>$info){
            $info['shops_name'] = $shops_names[$info['shops_code']];
    ...
    private function order_list_search() {
        return array(
            'url' => U('LabelDetailReport/index'),
            'button'=>array(
                '<a class="btn btn-success"  id="explode_order" data-url="'.U('LabelDetailReport/export').'"><i class="icon-download icon-white"></i> 导出报表</a>',
            ),
            'main' => array(
                 array(
                    'name' => 'payment_type[]',
                    'show_name' => '支付方式',
                    'select' => array(1=>'支付宝',2=>'银联支付',3=>'微信支付',4=>'文惠券',5=>'储值卡'),
                    'type' => 'select',
                     'multiple'=>true,
    ...
    private function get_where(){
        ...
        //储值卡
        if(in_array(5, $payment_type)){
            $where['real_pay_storage_money'] = array('gt',0);
            foreach($payment_type as $k=>$value){
                if($value == 5) unset ($payment_type[$k]);
            }
            $pay_bool = true;
        }
        ...
        if($payment_type) {
            $where['payment_type'] = array('in',$payment_type);
            $pay_bool = true;
        }
        if(!$pay_bool){
            $where['payment_type'] = array('neq',4);
        }
        ...
        if($pay_start && $pay_end) $where['pay_time'] = array('between',array($pay_start,$pay_end));
        $where['pay_cash'] = array('gt',0);
        return $where;
    }
