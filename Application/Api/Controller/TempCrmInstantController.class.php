...
class TempCrmInstantController extends BaseController{
    
    //订单同步
    public function index(){
        $this->syncBuyRecharge(1, 1);
    }
    ...
    private function syncBuyRecharge($pageIndex=1, $page_size=10){
        ...
        foreach($list as $k=>$order_goods_info){
            if($order_goods_info['order_goods_status'] == 8){
                $this->customAjaxReturn(201,false,'已经推送过数据，不要重复推送');
            }
...
//            $payment_info = array('payment_type'=>1);
    ...
    private function get_buy_recharge_where(){
        ...
        $where['info.order_cur_id'] = I('order_id','','trim');
        return $where;
    }
}
