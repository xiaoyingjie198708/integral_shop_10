 ...
 * @author xiao.yingjie
 */
class BenzSyncController extends BaseController{
    ...
    private function benz_code($pageIndex=1, $page_size=10){
        ...
        foreach($list as $k=>$order_goods){
            ...
            for($i =0;$i<$order_goods['goods_number'];$i++){
                //奔驰用车卡
	...
	if(($page_count > $pageIndex) && ($pageIndex * $page_size < 600))  {
            $this->benz_code(++$pageIndex, $page_size);
	}
	else{
            $log_content = json_encode(array('create_time'=>$last_date));
            \Org\My\MyLog::write_log($log_content, 'benz_code-info.log','','w'); 
            return true;
	}
    }
